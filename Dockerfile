...
CMD openssl req -new -newkey rsa:2048 -days 3650 -nodes -x509 \
...
CMD proto="`echo $REDIS_URL | grep '://' | sed -e's,^\(.*://\).*,\1,g'`"
CMD url=`echo $REDIS_URL | sed -e s,$proto,,g`

# Extract the user and password.
CMD auth="`echo $url | grep @ | cut -d@ -f1`"

# Extract the host and port.
CMD hostport=`echo $url | sed -e s,$auth@,,g | cut -d/ -f1`
